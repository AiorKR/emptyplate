...
public class KakaoPayService 
{
   ...
   public KakaoPayReady kakaoPayReady(KakaoPayOrder kakaoPayOrder)
   {
      ...
      if(kakaoPayOrder != null)
      {
         ...
         //Sprint Framework 에서 제공해주는 HttpEntity클래스 header와 Body를 합쳐
   ...
   public KakaoPayApprove kakaoPayApprove(KakaoPayOrder kakaoPayOrder)
   {
      ...
      if(kakaoPayOrder != null)
      {
         ...
         {
            logger.error("[KakaoService]kakaoPayApprove RestClientException", e);
         }
         catch(URISyntaxException e)
         {
            logger.error("[KakaoService]kakaoPayApprove URISyntaxException", e);
         }
      }
      else
      {
