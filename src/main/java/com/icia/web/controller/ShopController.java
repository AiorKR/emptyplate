	...
	public String shopView(ModelMap model, HttpServletRequest request, HttpServletResponse response) {
		...
		if (!StringUtil.isEmpty(shopUID) && !StringUtil.equals(shopUID, "")) {
			...
			for (int i = 0; i < shop.getShopTime().size(); i++) {
				if (Integer
						.parseInt(shop.getShopTime().get(i).getShopOrderTime().replaceAll(":", "")) >= standardTime) { // 시간에서
																														// :제거후
																														// int형으로
																														// 변환해서
																														// 기준시간과
				...
				if (reservationPeople > 0) {
					for (int i = 0; i < shopTotalTable.size(); i++) {
						for (int j = 0; j < shopTotalTable.get(i).getShopTable().size(); j++) {
							if (StringUtil.equals(shopTotalTable.get(i).getShopTable().get(j).getShopReservationTable()
									.getShopTableStatus(), "Y")) {
								count++;
							}
						}
						if (shopTotalTable.get(i).getShopTotalTable() == count) { // 예약된 테이블 갯수가 식당의 있는 테이블 수량과 같다면
							...
							shopTotalTable.get(i)
									.setShopTotalTableRmains(shopTotalTable.get(i).getShopTotalTable() - count); // 남아있는
																													// 자리
																													// 세팅
						}
					...
					if (count2 == shopTotalTable.size()) {
					...
					} else if (shopTotalTable != null && StringUtil.equals(counterSeatYN, "N")) { // 모든 자리를 카운터석으로만 예약할게
																									// 아니라면 모든 자리 조회,
																									// 예약인원이 1명 이상일 경우
						for (int i = 0; i < shopTotalTable.size(); i++) {
							if (reservationPeople % 2 == 0) { // 예약인원이 짝수 일때
								if (StringUtil.equals(shopTotalTable.get(i).getShopTotalTableStatus(), "N")) { // 자리가 있는
																												// 테이블
																												// 종류 확인
									if (shopTotalTable.get(i).getShopTotalTableRmains() >= (reservationPeople
											% shopTotalTable.get(i).getShopTotalTableCapacity())) {
						...
						for (int i = 0; i < shopTotalTable.size(); i++) {
							if (shopTotalTable.get(i).getShopTotalTableCapacity() == 1) { // 카운터석만 확인
								for (int j = 0; j < shopTotalTable.get(i).getShopTable().size(); j++) {
									if (StringUtil.equals(shopTotalTable.get(i).getShopTable().get(j)
											.getShopReservationTable().getShopTableStatus(), "Y")) { // 자리 조회 중
										count++;
									}
								}
								if (shopTotalTable.get(i).getShopTotalTable() == count) { // 예약된 테이블 갯수가 식당의 있는 테이블 수량과
																							// 같다면
									shopTotalTable.get(i).setShopTotalTableStatus("Y"); // 예약이 다 차있다면 Y를 세팅함. 디폴트값 N
									...
									shopTotalTable.get(i)
											.setShopTotalTableRmains(shopTotalTable.get(i).getShopTotalTable() - count); // 남아있는
																															...
																															// 숫자
																															// 세팅
									if (reservationPeople > shop.getShopTotalTable().get(i).getShopTotalTableRmains()) { // 1인테이블이므로
																															// 예약인원보다
																															// 남은
																															// 자리가
																															// 적으면
