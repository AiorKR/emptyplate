...
function fn_view(bbsTitle)
{
   document.bbsForm.bbsSeq.value = bbsTitle;
          ...
          <div class = "row">
              ...
              <div class="d-flex flex-row justify-content-between">
                   <div>
                       <ul>
                         <li>
                            <button type="button" value="1" id="btnSort" onclick="location.href='/board/list?bbsNo=${bbsNo}&searchType=${searchType}&searchValue=${searchValue}&sort=dateCount'" class="btn btn-outline-dark float-right" data-bs-toggle="button">최신순</button>
                         </li>
                         <li>   
                            <button type="button" value="2" id="btnSort"onclick="location.href='/board/list?bbsNo=${bbsNo}&searchType=${searchType}&searchValue=${searchValue}&sort=likeCount'" class="btn btn-outline-dark float-right">좋아요순</button>
                         </li>
                         <li>
                            <button type="button" value="3" id="btnSort"onclick="location.href='/board/list?bbsNo=${bbsNo}&searchType=${searchType}&searchValue=${searchValue}&sort=readCount'" class="btn btn-outline-dark float-right ">조회순</button>
                         </li>
                      </ul>
                 </div>
                   <div>
                     <select class="select" aria-label="Default select example">
                        <option value="1" selected <c:if test="${searchType eq '1'}">selected</c:if>>작성자</option>
                         <option value="2" <c:if test="${searchType eq '2'}">selected</c:if>>제목</option>
                         <option value="3" <c:if test="${searchType eq '3'}">selected</c:if>>내용</option>
                     </select>
                     <form action="" method="post">
                          <input type="text" name="_searchValue" id="_searchValue" value="${searchValue}"><input type="submit" id="btnSearch" value="검색">
                     </form>
                   </div>
              </div>
                     ...
                     <tr>
                  	...
                  	<c:if test="${!empty list}">
                        ...
                        <tr>
                           ...
                           <td><ion-icon name="heart"></ion-icon>&nbsp;</td>
                           <td class="likeNum">${board.bbsLikeCnt}</td>
                           <td><a href="javascript:void(0)" onclick="fn_view(${board.bbsTitle})">${board.bbsTitle}</a></td>
                           <td><a href="#">${board.userNick}</a></td>
                           <td>${board.bbsReadCnt}</td>
                           <td>${board.regDate}</td>
                        </tr>
